...
def plot_val_test_together(df_val, df_test, color_map, save_dir, tag, title="Validation vs Test Comparison"):
    ...
    plt.scatter(df_val["y_true"], df_val["y_pred"], color="gray", s=20, alpha=0.6, label="Validation")

    # Test points (colored by cell)
    plt.scatter(df_test["y_true"], df_test["y_pred"],
                c=df_test["CELL"].map(color_map), s=22, alpha=0.85, label="Test")
...
def main():
    ...
    for test_cell in cells:
        ...
        plot_dir = f"MLP_plots/LOOCV25{exp_tag}/{test_cell}/"
