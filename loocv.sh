...
USE_PARTIAL_SOC=true
USE_AGG_PERFORMANCE=false


# ----- Command arguments -----
FEATURES=("R0" "R1" "R2" "R3") # Note: Change as needed
INPUT_CSV="drtdf_date_G25SOC_idx.csv"


# ----- Build suffix based on toggle -----
SUFFIX=""
PARTIAL_SOC_FLAG=""
USE_AGG_PERFORMANCE_FLAG=""

if [ "$USE_PARTIAL_SOC" = true ]; then
    SUFFIX="_partialSOC"
    PARTIAL_SOC_FLAG="--partial_SOC"
fi
if [ "$USE_AGG_PERFORMANCE" = true ]; then
    USE_AGG_PERFORMANCE_FLAG="--print_agg"
fi
